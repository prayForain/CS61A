...
def prune_min(t):
    ...
    "*** YOUR CODE HERE ***"
    if not t.is_leaf():
        min_branch = min(t.branches, key=lambda b: b.label)
...
def num_splits(s, d):
    ...
    if not s:
        ...
        for i in range(1, len(s)):
            within_first += num_splits(s[1:i]+[s[i]-s[0]]+s[i+1:], d)
        without_first = num_splits(s[1:], d)
        return within_first + without_first
...
def foldr(link, fn, z):
    ...
    2
    >>> foldr(lst, add, 0) # (((0 + 1) + 2) + 3)
    6
    >>> foldr(lst, mul, 1) # (((1 * 1) * 2) * 3)
    6
    """
    if link is Link.empty:
        return z
