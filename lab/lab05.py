# Unsolved or Imperfect parts: riffle
#pass-test
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lata, lona = get_lat(city_a), get_lon(city_a)
    latb, lonb = get_lat(city_b), get_lon(city_b)
    return sqrt((lata - latb)**2 + (lona - lonb)**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a, dis_b = distance(city, city_a), distance(city, city_b)
...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    leaves_tree = [tree(leaves[i]) for i in range(len(leaves))]
    if is_leaf(t):
        return tree(label(t), leaves_tree)
    else:
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        bs = [add_trees(b[0], b[1]) for b in zip_longest(branches(t1), branches(t2), fillvalue= tree(0))]
