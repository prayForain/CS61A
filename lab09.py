...
def subseqs(s):
    ...
    if s == []:
        ...
        first, rest = s[0], s[1:]
        return subseqs(rest) + insert_into_all(first, subseqs(rest))
...
def make_generators_generator(g):
    ...
    lst = list(g())
    def gen(i):
        yield from [lst[index] for index in range(i)]
    yield from [gen(i) for i in range(1, len(lst)+1)]
...
def trade(first, second):
    ...
    while m <= len(first) and n <= len(second) and not sum(first[:m]) == sum(second[:n]):
