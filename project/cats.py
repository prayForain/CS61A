...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        return 0.0
    num, k = 0, 0
    mini_len = min(len(reference_words), len(typed_words))
    while k < mini_len:
...
def shifty_shifts(start, goal, limit):
    ...
    def helper(diff = 0, i = 0):
        if diff > limit:
            return limit + 1
        if i < length:
            if start[i] == goal[i]:
                return helper(diff, i + 1)
            else:
                return helper(diff + 1, i + 1)
        else:
            return diff
    return helper() + diff_len
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] != prompt[i]:
            send({'id': user_id, 'progress': i / len(prompt)})
            return i / len(prompt)
    send({'id': user_id, 'progress': len(typed)/len(prompt)})
...
def fastest_words(game):
    ...
    for player_index in player_indices:
        ...
        for word_index in word_indices:
            find_time = lambda player: time(game, player, word_index)
            if player_index == min(player_indices, key=find_time):
                words.append(word_at(game, word_index))
        fastest_words.append(words)
