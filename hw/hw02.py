...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(i, num, plus):
        if i == n:
            return num
        if (i + 1) % 8 == 0 or num_eights(i + 1) > 0:
            return helper(i + 1, num + plus, -plus)
        else:
            return helper(i + 1, num + plus, plus)
    return helper(1, 1, 1)
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        last1 = n % 10
        last2 = (n // 10) % 10
        if last2 == last1 or last2 == last1 - 1:
...
def count_coins(total):
    ...
    def helper(total, smallest_coin):
        if total == 0:
            ...
            without_smallset_coin = helper(total, next_coin)
            return with_smallest_coin + without_smallset_coin
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda b: (lambda f, b: f(f,b))((lambda f, b: 1 if b==1 else mul(b, f(f,sub(b,1)))), b)
