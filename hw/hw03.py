...
def mul_interval(x, y):
    ...
    upper_bound_x = upper_bound(x)
    upper_bound_y = upper_bound(y)
    p1 = lower_x * lower_y
    p2 = lower_x * upper_bound_y
    p3 = upper_bound_x * lower_y
    p4 = upper_bound_x * upper_bound_y
...
def multiple_references_explanation():
    ...
    """For the same interval, they should take same value.But the mul_interval function take different values."""
...
def quadratic(x, a, b, c):
    ...
    minimum = min(f(lower_x), f(upper_x))
    maximum = max(f(lower_x), f(upper_x))
    if d < upper_x and d > lower_x:
        if f(d) > f(lower_x) and f(d) > f(upper_x):
            return interval(minimum, f(d))
        else:
            return interval(f(d), maximum)
    else:
